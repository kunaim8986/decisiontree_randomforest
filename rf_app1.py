...
st.set_page_config(page_title="Tree / Random Forest Classification App", layout="wide")

st.title("Decision Tree & Random Forest Classification Web App")
st.write(
    "Upload a dataset, select features & target, train a Decision Tree or Random Forest classifier, "
...
model_choice = st.sidebar.radio("Choose model", ["Decision Tree", "Random Forest"])
...
if model_choice == "Decision Tree":
    ...
    max_depth_val = None if max_depth_param == 0 else int(max_depth_param)
...
# Random forest params
    ...
    rf_max_depth_param = st.sidebar.slider("Max depth (None = 0)", 0, 50, 10, 1)
    rf_max_depth_val = None if rf_max_depth_param == 0 else int(rf_max_depth_param)
    rf_min_samples_split = st.sidebar.number_input("min_samples_split", min_value=2, value=2, step=1)
    rf_min_samples_leaf = st.sidebar.number_input("min_samples_leaf", min_value=1, value=1, step=1)
    max_features = st.sidebar.selectbox("max_features", ["auto", "sqrt", "log2"], index=1)
...
if model_choice == "Decision Tree":
    ...
    st.markdown("Random Forest: feature importances shown above.")
    if st.checkbox("Show one tree from the forest (Graphviz)"):
        tree_idx = st.number_input("Estimator index", min_value=0, max_value=len(model.estimators_) - 1, value=0, step=1)
        dot_data = export_graphviz(
            model.estimators_[int(tree_idx)],
            out_file=None,
            feature_names=feature_cols,
            class_names=[str(c) for c in display_class_names],
            filled=True,
